...
/*{{{*/ {
    uint8_t i;
    uint8_t channels[PWM_CHANNELS] = {1<<5, 1<<6, 1<<7};
    init_timer1();
    global_pwm.dim = 255;
    ...
    for (i=0; i<PWM_CHANNELS; i++) {
        ...
        global_pwm.channels[i].pinmask = channels[i];
...
#if LED_PORT_INVERT
    LED_PORT |= PWM_CHANNELMASK;
#else
    LED_PORT &= ~PWM_CHANNELMASK;
#endif

    /* configure Px0-Px2 as outputs */
    LED_PORT_DDR |= PWM_CHANNELMASK;
}
...
/*{{{*/ {
    ...
    for (i=0; i < PWM_CHANNELS; i++) {
        ...
        if (global_pwm.channels[index].brightness_scale > 0 && global_pwm.channels[index].brightness_scale < 255) {
            ...
            if (global_pwm.channels[index].brightness_scale > last_brightness) {
                ...
                mask |= global_pwm.channels[index].pinmask;
    ...
    for (i=0; i < PWM_CHANNELS; i++)
        if (global_pwm.channels[i].brightness_scale > 0)
            pwm.initial_bitmask &= ~global_pwm.channels[i].pinmask;
} /*}}}*/
...
/*{{{*/ {
...
#if LED_PORT_INVERT
        LED_PORT &= ~PWM_CHANNELMASK;
        LED_PORT |= pwm.initial_bitmask & PWM_CHANNELMASK;
#else
        LED_PORT &= ~PWM_CHANNELMASK;
        LED_PORT |= (~pwm.initial_bitmask)&PWM_CHANNELMASK;
#endif
...
#if LED_PORT_INVERT
        LED_PORT |= pwm.next_bitmask & PWM_CHANNELMASK;
#else
        LED_PORT &= ~(pwm.next_bitmask & PWM_CHANNELMASK);
#endif
    /* output values */
    ...
    while( pwm.index < pwm.count && TCNT1 + 128 > pwm.slots[pwm.index].top)
    {
        ...
        /* output value */
#if LED_PORT_INVERT
        LED_PORT |= pwm.slots[pwm.index].mask & PWM_CHANNELMASK;
#else
        LED_PORT &= ~(pwm.slots[pwm.index].mask & PWM_CHANNELMASK);
#endif
        pwm.index++;
